...
- Click **Add Container**
- Toggle **Basic View** to **OFF** (so you see advanced options)
...
```bash
docker run --rm -v /mnt/user/appdata/frigate:/output python:3.11 bash -c "apt-get update && apt-get install -y libgl1 libglib2.0-0 && pip install ultralytics onnx onnxsim && yolo export model=yolov9t.pt format=onnx imgsz=320 simplify=True opset=17 && cp yolov9t.onnx /output/"
```

### What This Command Does

- **`docker run --rm`** - Creates a temporary container that auto-deletes when done
- **`-v /mnt/user/appdata/frigate:/output`** - Maps your Frigate config folder into the container
- **`python:3.11`** - Uses official Python 3.11 image (clean environment)
- **`apt-get install -y libgl1 libglib2.0-0`** - Installs OpenCV dependencies (fixes `libGL.so.1` error)
- **`pip install ultralytics onnx onnxsim`** - Installs YOLO export tools
- **`yolo export model=yolov9t.pt format=onnx imgsz=320 simplify=True opset=17`** - Downloads YOLOv9-tiny and exports to ONNX
- **`cp yolov9t.onnx /output/`** - Copies the model to your Frigate config folder
...
```bash
docker run --rm -v /mnt/user/appdata/frigate:/output python:3.11 bash -c "apt-get update && apt-get install -y libgl1 libglib2.0-0 && pip install ultralytics onnx onnxsim && yolo export model=yolov9t.pt format=onnx imgsz=640 simplify=True opset=17 && cp yolov9t.onnx /output/"
```
...
# Explicitly specify GPU device for ONNX inference
# =============================================================================
detectors:
  onnx:
    type: onnx
    device: 0    # Use first GPU (required for explicit GPU selection)

# =============================================================================
# HARDWARE ACCELERATION
# Use NVIDIA for video decoding - auto-detects H.264/H.265
# =============================================================================
ffmpeg:
...
The `device: 0` parameter explicitly tells the ONNX runtime to use the first GPU. While the `stable-tensorrt` image can auto-detect CUDA, specifying the device ensures consistent GPU usage.
...
Using `preset-nvidia` (without `-h264` or `-h265`) allows ffmpeg to **automatically detect** the correct codec for your camera streams. This works with both H.264 and H.265/HEVC cameras without needing separate configurations.
...
- Look at the **Detectors** section
- Verify inference speed is ~5ms (GPU) not ~50ms+ (CPU fallback)
...
**Error:**
```
your_camera exceeded fps limit. Exiting ffmpeg... [rtsp @ ...] RTP: PT=61: bad cseq ...
```

**Causes:**
- Camera sub-stream FPS is too high
- Another application (e.g., AgentDVR) is fighting for the same camera streams
...
**Error:**
```
ONNX: loading None TypeError: Unable to load from type '<class 'NoneType'>'
```
...
**Check 4:** Verify `device: 0` is set in your detectors config.
...
- [ ] config.yml has `device: 0` in detectors section
- [ ] config.yml uses `hwaccel_args: preset-nvidia` (auto-detects codec)
